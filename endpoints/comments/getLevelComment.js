	...
	async execute(req, res, body, server) {
		...
		if (!comments || !commentsCount) {
			...
			for (const comment of comments) {
				...
				commentsString += `2~${comment.comment}~3~${comment.accountID}~4~${comment.likes}~5~0~7~${comment.isSpam}~9~${dateAgo}~6~${comment.commentID}~10~${comment.percent}`;
				commentsString += `~11~${user.modLevel}~12~${user.textColor}:1~${user.userName}~7~1~9~${user.icon}~10~${user.color1}~11~${user.color2}~14~${user.iconType}~15~${user.special}~16~${user.accountID}|`;
			};
