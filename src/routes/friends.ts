...
function routes(app: tinyhttp) {
	app.all(`/${config.basePath}/getGJFriendRequests20`, async (req: any, res: any) => {
		...
		if (!requests) {
			...
			return res.send(requestsList.join('|') + `#${requests.length}:${page * 10}:10`);
		}
	});

	app.all(`/${config.basePath}/getGJUserList20`, async (req: any, res: any) => {
		...
		if (await GJCrypto.gjpCheck(gjp, accountID)) {
			...
			return res.send(usersList.join('|'));
		} else {
	...
	app.all(`/${config.basePath}/acceptGJFriendRequest20`, async (req: any, res: any) => {
	...
	app.all(`/${config.basePath}/blockGJUser20`, async (req: any, res: any) => {
	...
	app.all(`/${config.basePath}/unblockGJUser20`, async (req: any, res: any) => {
		const requredKeys = ['gjp', 'accountID', 'targetAccountID', 'secret'];
		const body = req.body;
		if (!WebHelper.checkRequired(body, requredKeys, res)) return;
	...
	app.all(`/${config.basePath}/uploadFriendRequest20`, async (req: any, res: any) => {
	...
	app.all(`/${config.basePath}/deleteGJFriendRequests20`, async (req: any, res: any) => {
	...
	app.all(`/${config.basePath}/readGJFriendRequest20`, async (req: any, res: any) => {
